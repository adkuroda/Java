...
class Simulator {
	...
	/**
	 * Sets up the position of the train based on given text input. Input 
	 * gets split into token using space as delimiter and assigns the train 
	 * name and all the wagons to the train. 
	 * @param trainScanner reads the input from text file. 
	 */
	public void setupInitialTrainPositions(Scanner trainScanner) {
		while(trainScanner.hasNextLine()) {
			String temp = trainScanner.nextLine();
			if (temp.length() == 0) {
				return;
			}
			String[] chooAndWagon = temp.split(" ");
			Train chooChoo = new Train(chooAndWagon[0]);
			trains.append(chooAndWagon[0], chooChoo);
		
			for (int i = 1; i<chooAndWagon.length; i++) {
				Car wagon = new Car(chooAndWagon[i]);
				chooChoo.connectCar(wagon);
				cars.append(chooAndWagon[i], wagon);
			}
		}
	}
	 ...
	 *  @param murderLocation location where the murder happened. 
	 *  @param murderTimeStr list of possible suspects for the murder. 
	 ...
	 *  @param murderLocation with the name of the car the murder takes place at.
	 *  @return a set of potential murderers.
	 */
	 ...
	 *  @param carObj a car or wagon. 
	 *  @return list of the people on the car
	 */
